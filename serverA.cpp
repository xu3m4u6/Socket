...
#define UDP_PORT_A "30718" //server A UDP port number
...
struct addrinfo hints, *serverMainInfo, *serverAInfo; //address info
...
void start_serverA(){
    ...
    if ((sockfd_UDP = socket(serverAInfo->ai_family, serverAInfo->ai_socktype, serverAInfo->ai_protocol)) == -1) {
        perror("server A: fail to create socket");
    }
    // bind
    if (bind(sockfd_UDP, serverAInfo->ai_addr, serverAInfo->ai_addrlen) == -1) {
        close(sockfd_UDP);
        perror("server A: binding failed");
    }
    cout << "The server A is up and running using UDP on port <" << UDP_PORT_A << ">" << endl;
}
...
void send_countrylist() {
    ...
    {
        perror("server A: fail to recvfrom main server's country list request");
        exit(1);
    }
    buf[numbytes] = '\0';
    ...
    if ((numbytes = sendto(sockfd_UDP, msg.c_str(), MAXBUFLEN, 0,
        ...
        perror("server A: fail to send country list to main server");
        exit(1);
    }
...
void listen_and_respond(){
    ...
    if ((numbytes = sendto(sockfd_UDP, result.c_str(), result.length(), 0,
        serverMainInfo->ai_addr, serverMainInfo->ai_addrlen)) == -1) {
        perror("server A: fail to send to main with results");
        exit(1);
    }
